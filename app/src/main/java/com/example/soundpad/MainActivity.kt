...
class MainActivity : AppCompatActivity() {
    ...
    private val grayColor = android.graphics.Color.parseColor("#757575") // Gray color for playing

    private val soundPicker = registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
        uri?.let {
            // Take persistent permission to maintain URI access across app restarts
            try {
                // Only use READ permission, don't use PERSISTABLE flag here
    ...
    private fun setupStopButtons() {
        // Set the stop buttons to the specified color
    ...
    private fun stopPlayingSound() {
        ...
        viewModel.stopPlayingSound() // Notify ViewModel that sound stopped playing
    }
    ...
    private fun setupSoundGrid() {
        ...
        buttonContainers.forEachIndexed { index, container ->
            ...
            // Hide favorite button for buttons without sounds
        ...
        viewModel.sounds.value?.forEach { sound ->
            if (sound.id < buttonContainers.size) {
                ...
                // Normal click to play the sound
                soundButton.setOnClickListener {
                ...
                // Show and update favorite button - make sure it's visible
                favoriteButton.visibility = View.VISIBLE
                
                // Make sure to update the star icon based on favorite status
                ...
                favoriteButton.setOnClickListener {
                    ...
                    // Immediately update the appearance of the star
    ...
    private fun setupNavigation() {
        // Set icon color to match buttons
    ...
    private fun setupFavoritesRecyclerView() {
        ...
        binding.favoritesRecyclerView.apply {
            // Use a grid layout with 2 columns, identical to the main soundpad
            val gridLayoutManager = androidx.recyclerview.widget.GridLayoutManager(this@MainActivity, 2)
            layoutManager = gridLayoutManager
            
            // Add item decoration to match the custom margins from GridLayout
            addItemDecoration(object : androidx.recyclerview.widget.RecyclerView.ItemDecoration() {
                override fun getItemOffsets(
                    ...
                    // Apply 4dp horizontal spacing between items but not on outer edges
                    outRect.left = if (column == 0) 0 else 4
                    outRect.right = if (column == 1) 0 else 4
                    
                    // Apply 8dp bottom margin to all items
                    outRect.top = 0
                    outRect.bottom = 8
                }
            })
    ...
    private fun updateFavoriteButtonAppearance(favoriteButton: ImageButton, isFavorite: Boolean) {
        // Make sure we use the correct filled/hollow star icons
        favoriteButton.setImageResource(
            if (isFavorite) R.drawable.ic_star_filled 
            else R.drawable.ic_star_hollow
        )
        
        // Make sure the favorite button is in front of other elements
    ...
    private fun playSound(sound: SoundItem) {
        ...
        try {
            ...
            mediaPlayer?.apply {
                ...
                // When playback completes, clear the playing state
            ...
            viewModel.stopPlayingSound() // Clear playing state on error
            
            // If this sound is from a previous session and URI permission was lost
            // Let the user know they can long-press to replace it
            val buttonContainers = listOf(
                binding.btn00, binding.btn01, binding.btn02, binding.btn03, binding.btn04,
                binding.btn10, binding.btn11, binding.btn12, binding.btn13, binding.btn14,
                binding.btn20, binding.btn21, binding.btn22, binding.btn23, binding.btn24,
            ...
            if (sound.id < buttonContainers.size) {
                // Already handled by updateSoundButtons
            }
        }
    }
